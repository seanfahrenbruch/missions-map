      ...
      {mission.status === "upcoming" && (
        <div className="p-5 border-b border-gray-800">
          <div className="rounded-xl p-4 text-center" style={{ background: `${cfg.color}15`, border: `1px solid ${cfg.color}40` }}>
            ...
            <p className="text-xs text-gray-400 mt-1">This site is in development. Check back for updates on orphans and widows served.</p>
          </div>
        </div>
      )}
...
function StatsBar({ typeFilter }: { typeFilter: MarkerType | "all" }) {
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[500] px-5 py-3 rounded-xl bg-gray-950/95 border border-gray-800 text-white shadow-xl backdrop-blur-sm flex gap-5 items-center whitespace-nowrap">
      ...
      <div className="w-px h-8 bg-gray-700" />
      <div className="text-center">
        <div className="text-xl font-bold text-white tabular-nums">{total.toLocaleString()}</div>
...
function Legend({ statusFilter, setStatusFilter }: {
  ...
  return (
    <div className="absolute bottom-8 left-4 z-[500] rounded-xl text-xs bg-gray-950/95 border border-gray-800 shadow-xl backdrop-blur-sm overflow-hidden">
        ...
        {(["all", "active", "upcoming"] as const).map((key) => {
          ...
          return (
            <button key={key}
              ...
              className={`flex items-center gap-2.5 w-full rounded-lg px-2.5 py-1.5 transition-colors ${active ? "bg-gray-800" : "hover:bg-gray-900/60"}`}>
              {isAll ? (
                <span className="w-3 h-3 rounded-full bg-gray-500 shrink-0" />
              ) : (
                <span className="relative inline-block w-3 h-3 shrink-0">
                  <span className="absolute inset-0 rounded-full opacity-50 animate-pulse" style={{ background: cfg!.color }} />
      ...
      MISSIONS
        ...
        .forEach((mission) => {
          ...
          const icon = L.divIcon({
            className: "",
            html: `
              <div class="wc-marker" style="--clr:${cfg.color}">
